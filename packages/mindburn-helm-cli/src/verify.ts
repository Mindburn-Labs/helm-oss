...
async function checkSignature(
    ...
    // Hash-based signature check (self-consistency)
    ...
    const expectedSig = sha256Hex(Buffer.from(payload));

    if (expectedSig !== sig.signature) {
        return { pass: false, reason: "signature payload mismatch" };
    }
