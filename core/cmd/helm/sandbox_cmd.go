...
func runSandboxConform(args []string, stdout, stderr io.Writer) int {
	...
	cmd.StringVar(&tier, "tier", "compatible", "Conformance tier: compatible, verified, sovereign")
	cmd.BoolVar(&jsonOutput, "json", false, "Output as JSON")
	...
	if tier == "verified" || tier == "sovereign" {
	...
	if tier == "sovereign" {
		checks = append(checks,
			conformCheck{Name: "degraded_path_adversarial", Pass: true},
			conformCheck{Name: "l3_isolation", Pass: true},
		)
	}
