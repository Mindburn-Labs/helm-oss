...
(function() {
  ...
  var denyExplanations = {
    ...
    'DENY_BUDGET_EXCEEDED': {human:'Operation would exceed the configured budget ceiling.',clause:'policy.budget.ceiling',fix:'Increase the budget ceiling or reduce operation scope.'},
    'DENY_SCHEMA_MISMATCH': {human:'Input or output schema does not match the registered contract.',clause:'policy.schema_enforcement',fix:'Update the tool schema or fix the payload to match.'},
    'DENY_GAS_EXHAUSTION': {human:'Sandbox execution exceeded gas/time/memory budget.',clause:'policy.sandbox.limits',fix:'Increase sandbox resource limits or optimize the workload.'},
    'DENY_APPROVAL_REQUIRED': {human:'This action requires explicit approval before execution.',clause:'policy.approval_gates',fix:'Request approval via helm approve or from an authorized principal.'},
