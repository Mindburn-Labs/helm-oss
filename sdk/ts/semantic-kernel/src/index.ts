 ...
 * Integrates with SK's KernelFunction and KernelPlugin patterns.
 *
 * @example
 * ```typescript
 ...
 * const governed = governor.governPlugin(myPlugin);
 * ```
 */

export interface HelmSKConfig {
  helmUrl: string;
  apiKey?: string;
  failClosed?: boolean;
...
export class HelmToolDenyError extends Error {
  ...
  constructor(denial: ToolCallDenial) {
    super(
      `HELM denied "${denial.pluginName}.${denial.toolName}": ${denial.reasonCode} — ${denial.message}`,
    );
...
export class HelmSKGovernor {
  private config: Required<HelmSKConfig>;
  ...
  constructor(config: HelmSKConfig) {
    this.config = {
  ...
  /**
   * Wrap a Semantic Kernel plugin (map of function name → function) with HELM governance.
   */
  governPlugin(
    pluginName: string,
    functions: Record<string, (...args: any[]) => any>,
  ): Record<string, (...args: any[]) => any> {
    const governed: Record<string, (...args: any[]) => any> = {};
    for (const [fnName, fn] of Object.entries(functions)) {
      governed[fnName] = this.governFunction(pluginName, fnName, fn);
    }
    return governed;
  }

  /**
   * Wrap a single kernel function with HELM governance.
   */
  governFunction<T extends (...args: any[]) => any>(
    pluginName: string,
    functionName: string,
    fn: T,
  ): T {
    ...
    const governed = async function (...args: any[]) {
      ...
      try {
        const response = await governor.evaluateIntent(
