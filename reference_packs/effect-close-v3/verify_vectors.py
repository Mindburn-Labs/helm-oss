#!/usr/bin/env python3
"""Independent connector acknowledgement v3 and Kernel close v3 verifier.
...
def validate_acknowledgement(acknowledgement):
    if (
        acknowledgement.get("schema_version") != "connector-effect-acknowledgement.v3"
        or acknowledgement.get("contract_version") != "2026-09-06"
    ...
    if "disposition_receipt_hash" in acknowledgement:
        try:
            ...
            require_token(acknowledgement.get("reconciliation_ref"), "reconciliation_ref")
        except VectorError as error:
...
def verify_acknowledgement(index, root, acknowledgement, envelope, signature_value):
    ...
    expected_payload = {
        ...
        "domain": "HELM/ConnectorEffectAcknowledgementSignature/v3",
        "issuer_id": acknowledgement["issuer_id"],
        "signing_key_ref": acknowledgement["signing_key_ref"],
    }
    if "disposition_receipt_hash" in acknowledgement:
        expected_payload["disposition_receipt_hash"] = acknowledgement["disposition_receipt_hash"]
...
def validate_receipt(receipt):
    if (
        receipt.get("schema_version") != "effect-close-receipt.v3"
        or receipt.get("contract_version") != "2026-09-06"
...
def verify_receipt(index, root, receipt, acknowledgement, signature_value):
    ...
    expected_payload = {
        ...
        "domain": "HELM/EffectCloseReceiptSignature/v3",
    ...
    if "disposition_receipt_hash" in receipt:
        expected_payload["disposition_receipt_hash"] = receipt["disposition_receipt_hash"]
    if payload != expected_payload:
...
def verify_vector(index, root, mutation=None):
    ...
    if mutation == "set_acknowledgement_disposition_hash_to_tampered":
        acknowledgement["disposition_receipt_hash"] = "sha256:" + "9" * 64
        envelope["acknowledgement"]["disposition_receipt_hash"] = acknowledgement["disposition_receipt_hash"]
    elif mutation == "set_receipt_disposition_hash_to_other_and_reseal":
        receipt["disposition_receipt_hash"] = "sha256:" + "8" * 64
        reseal(receipt, "receipt_hash")
    elif mutation == "remove_receipt_disposition_hash_and_reseal":
        receipt.pop("disposition_receipt_hash")
        reseal(receipt, "receipt_hash")
    elif mutation == "remove_acknowledgement_reconciliation_ref_and_reseal":
        acknowledgement.pop("reconciliation_ref")
        reseal(acknowledgement, "acknowledgement_hash")
        envelope["acknowledgement"] = copy.deepcopy(acknowledgement)
    elif mutation == "set_acknowledgement_activation_record_hash_to_tampered":
...
def main():
    ...
    if index.get("schema_version") != "effect-close-vectors.v3" or index.get("contract_version") != "2026-09-06":
        raise SystemExit("unsupported effect close v3 vector contract")
    ...
    print(
        "verified effect close v3 vectors: "
